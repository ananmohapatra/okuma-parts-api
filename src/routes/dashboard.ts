...
async function getDealerCustomerIds(dealerId: number): Promise<number[]> {
    ...
    if (companyName) {
        ...
        const matchedGroup = groupsRes.data.find((g: any) => g.name === companyName);

        if (matchedGroup) {
            const custRes = await bcClient.get(`/v3/customers?customer_group_id:in=${matchedGroup.id}&limit=250`);
            const groupIds: number[] = custRes.data.data.map((c: any) => c.id);
...
router.get('/recent-orders', async (req: Request, res: Response) => {
    try {
        ...
        allOrders.sort((a: any, b: any) => new Date(b.date_created).getTime() - new Date(a.date_created).getTime());

        const totalOrderCount = allOrders.length;
        const openOrderCount = allOrders.filter((o: any) => OPEN_STATUS_IDS.has(o.status_id)).length;

        const recentOrders = allOrders.slice(0, limit).map((o: any) => ({
            orderId: o.id,
        ...
        logger.error(`Dashboard recent-orders error: ${err.message}`);
...
router.get('/quotes', async (req: Request, res: Response) => {
    try {
        ...
        const allQuotes: any[] = quotesRes.data?.data ?? [];

        const dealerQuotes = allQuotes.filter(q => {
            const field = (q.extraFields ?? []).find((f: any) => f.fieldName === 'Customer Account ID');
            return field && customerIdSet.has(String(field.fieldValue));
        });

        const openQuoteCount = dealerQuotes.length;
        const data = dealerQuotes.slice(0, limit).map((q: any) => ({
            quoteId: q.quoteId,
        ...
        logger.error(`Dashboard quotes error: ${err.message}`);

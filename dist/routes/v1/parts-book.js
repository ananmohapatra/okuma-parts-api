...
async function fetchCategoryImages(categoryIds) {
    ...
    try {
        ...
        logger_1.default.error(`parts-book: category image lookup failed: ${err.message}`);
        return {};
    }
}
// ---------------------------------------------------------------------------
...
router.get('/customer/:customerId/machines', async (req, res, next) => {
    ...
    try {
        ...
        try {
            ...
            logger_1.default.error(`customer ${customerId}: registered_machines metafield is not valid JSON`);
            return res.json({ machines: [] });
        }
